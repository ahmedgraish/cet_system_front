...
import type { StudentAnswers } from '@/repository/interfaces';
...
const quizData = ref<StudentAnswers[]>([])
const getQuizData = async () => {
    quizData.value = await teacherStore.getStudentsResults(Number(route.params.quizId), Number(route.params.studentId))
}
getQuizData()

let activeQuestion = ref(0)
const NextQuestion = () => {
    activeQuestion.value < quizData.value!.length - 1 ? activeQuestion.value++ : ''
                ...
                <CardContent class="relative h-full flex flex-col items-center justify-around ">
                    ...
                    <div class="w-[95%] md:w-[80%]  flex flex-col items-center gap-5 mt-2 font-Somar font-semibold"
                        ...
                        <div id="answer" :key="quizData![activeQuestion].options.indexOf(option)"
                            ...
                            :class="showResults(option, quizData![activeQuestion].user_answer, quizData![activeQuestion].model_answer)"
                    ...
                    <CardFooter class="w-full flex items-center justify-center mt-10">
                        <Button type="button"
                            @click="activeQuestion === quizData!.length - 1 ? router.push({ name: 'teacherQuizPreview' }) : NextQuestion()"
