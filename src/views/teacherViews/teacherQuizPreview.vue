        ...
        <main
            ...
            <div class="w-[95%] md:w-5/6 h-fit pb-24 flex flex-col items-center ">
                ...
                <DataTable class="w-full mt-3 ">
                    <template #header>
                        <h1 class="w-2/6 self-start h-full pr-3">رقم قيد الطالب</h1>
                        <h1 class="w-3/6">اسم الطالب</h1>
                        <h1 class="w-1/6 text-center">الدرجة</h1>
                    </template>
                    <template #row>
                        <router-link :to="{ name: 'studentQuizResult', params: { studentId: '' } }">
                            <div :key="index" v-for="student, index in students"
                                class="w-full flex min-h-16 items-center pr-3 hover:bg-curious-blue-50 hover:cursor-pointer">
                                <h1 class="w-2/6">{{ student.ref }}</h1>
                                <h1 class="w-3/6">{{ student.name }}</h1>
